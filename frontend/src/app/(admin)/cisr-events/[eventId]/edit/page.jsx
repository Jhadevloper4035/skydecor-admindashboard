...
const EditCisrEvent = () => {
  ...
  const { events, loading, fetchEvents, updateEvent } = useCisrEventStore();
  ...
  const onSubmit = async (values) => {
    ...
    setSaving(false);
    if (result) navigate(`/cisr-events/${eventId}`);
  };
  ...
  return (
    <>
      <PageMetaData title={`Edit CISR Event ${event?.title ?? ''}`} />
      <PageBreadcrumb title="Edit CISR Event" subName="CISR Events" />
            ...
            <CardBody>
              <form onSubmit={handleSubmit(onSubmit)}>
                ...
                <div className="d-flex gap-2 mt-2">
                  ...
                  <Link to={`/cisr-events/${eventId}`} className="btn btn-outline-secondary">
                    Cancel
                  </Link>
                </div>
              </form>
