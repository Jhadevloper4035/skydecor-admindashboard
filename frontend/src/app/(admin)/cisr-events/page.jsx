...
const CisrEventCard = ({ event, onDelete, deleting }) => {
    ...
    <Col xs={12} sm={6} md={4} xl={3}>
      <Card className="h-100 shadow-none border">
        <div style={{ height: 170, overflow: 'hidden', background: '#f8f9fa' }} className="rounded-top">
          {event.coverImage ? (
            <img src={event.coverImage} alt={event.title} className="w-100 h-100" style={{ objectFit: 'cover' }} />
          ) : (
        ...
        <div className="border-top p-2 d-flex gap-1">
          <Link to={`/cisr-events/${event._id}`} className="btn btn-sm btn-soft-info flex-fill py-1" title="View">
            <IconifyIcon icon="bx:show" />
          </Link>
          <Link to={`/cisr-events/${event._id}/edit`} className="btn btn-sm btn-soft-secondary flex-fill py-1" title="Edit">
...
const CisrEvents = () => {
  const { events, loading, fetchEvents, deleteEvent } = useCisrEventStore();
  ...
  const handleDelete = async (event) => {
    ...
    if (ok) {
      await Swal.fire({
        title: 'Deleted',
        text: 'The CISR event was deleted successfully.',
        icon: 'success',
        timer: 1500,
  ...
  return (
    <>
      <PageMetaData title="CISR Events" />
      <PageBreadcrumb title="CISR Events" subName="Website Utilities" />
          ...
          <div className="d-flex flex-wrap justify-content-between gap-3">
            <div className="search-bar">
              ...
              <input
                ...
                placeholder="Search CISR events..."
                value={search}
                onChange={(e) => setSearch(e.target.value)}
              />
            </div>
            <Link to="/cisr-events/create" className="btn btn-primary d-flex align-items-center">
      ...
      {loading ? (
        ...
        <div className="text-center text-muted py-5">No CISR events found</div>
      ) : (
        <Row className="g-3">
          {sorted.map((event) => (
            <CisrEventCard
