...
const ProductEnquiriesTable = () => {
  ...
  const columns = useMemo(
    () => [
      ...
      { accessorKey: 'productInterest', header: 'Product', cell: ({ getValue }) => getValue() },
      ...
      { accessorKey: 'timeElapsed', header: 'Time', cell: ({ getValue }) => getValue() },
      {
      ...
      },
      {
        ...
        header: 'Status',
        cell: ({ getValue }) => (
          <span className={`badge bg-${statusVariant[getValue()] ?? 'soft-secondary'}`}>{getValue() ?? 'pending'}</span>
        ),
      },
      {
        id: 'actions',
      ...
      <Card.Header>
        <div className="d-flex justify-content-between align-items-center">
          ...
          <InputGroup style={{ width: '300px' }}>
            <Form.Control type="text" placeholder="Search enquiries..." value={globalFilter ?? ''} onChange={(e) => setGlobalFilter(e.target.value)} />
        ...
        <table className="table text-nowrap mb-0">
          ...
          <tbody>
            ...
            {table.getRowModel().rows.length === 0 && (
              <tr><td colSpan={columns.length} className="text-center text-muted py-4">No product enquiries found.</td></tr>
            )}
          </tbody>
        </table>
      </div>
